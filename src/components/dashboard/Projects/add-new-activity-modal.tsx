...
import { Input, Modal, Button, ModalContent, ModalFooter, ModalHeader, Select, SelectItem, Textarea, Chip, Avatar } from '@nextui-org/react'
...
const AddNewActivityModal = ({ id }: { id: string }) => {
  ...
  const addActivity = useMutation({
    mutationFn: async (data: any) => {
      return postData(activityRoutes.create + id, {}, data)
    },
    onSuccess: () => {
      ...
      <Modal isOpen={open} onClose={() => closeModal()} className='' size='lg'>
        ...
        <ModalContent className='p-5'>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Input type='date' variant='bordered' placeholder='Target date' className='py-2' id='target' />

            <div className='flex flex-wrap gap-2'>
              {/* if empty array , put a block */}
              {
                workers.length === 0 && <div className='text-gray-400'>No workers added yet</div>
              }
              {
                workers.map((worker: any, index: number) => {
                  return <Chip
                    ...
                    key={index}
                    onClose={() => {
                      setWorkers(workers.filter((w: any) => w._id !== worker._id))
                    }}
                    avatar={
                      <Avatar name={worker.name} size="sm" getInitials={(name: any) => name.charAt(0)} />
                    }
                  >
                    {worker.name}
                  </Chip>
            ...
            <Select variant='bordered' placeholder='Select user' onChange={(e: any) => {
            }} className='py-2' id='selectuser'>
              {
                userByRoleData.data?.data?.data.filter(
                  (user: any) => !workers.find((worker: any) => worker._id === user._id)
                ).map((user: any) => {
                  return <SelectItem onClick={(e: any) => {
                    setWorkers([...workers, user])

                  }} value={user._id} key={user._id}>{user.name}</SelectItem>
                })
              }
            </Select>
