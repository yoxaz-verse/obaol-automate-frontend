...
const VerifyActivityModal = ({open,close,activity}:{open:boolean,close:()=>void,activity:any}) => {
  return (
    ...
    <div className='flex justify-between w-full items-center pb-4'>
      <div className='text-[#3EADEB] text-lg font-medium'>{activity.projectName}</div>
    </div>
    <div>
      {/* Render project manager */}
      <div className='flex'>
        <Avatar src={activity.projectManager.avatar} alt='avatar' size='lg'/>
        <div className='flex flex-col px-2'>
          <div className='font-medium'>{activity.projectManager.name}</div>
          <div className='text-[#A1A1AA] text-sm'>{activity.projectManager.role}</div>
        </div>
      </div>
     
      <div className='text-[#A1A1AA] text-sm pt-4 pb-8'>{activity.description}</div>
