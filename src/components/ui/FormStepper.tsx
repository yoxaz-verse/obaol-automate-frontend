  ...
  return (
    <nav aria-label="Form progress" className="mb-6">
      ...
      <ol className="grid gap-2" style={{ gridTemplateColumns: `repeat(${steps.length}, minmax(0, 1fr))` }}>
        {steps.map((step, index) => <li key={step} aria-current={index + 1 === currentStep ? "step" : undefined} className={`h-2 rounded-full ${index + 1 <= currentStep ? "bg-warning-500" : "bg-default-200"}`}><span className="sr-only">{step}</span></li>)}
      </ol>
    </nav>
  );
}
